extends Move
class_name Slash2   # <-- rename per attack (Fabric dash .... )

# ready: identify this move
const COMBO_TIMING := 0.6     # how early a queued combo can fire (usually mid-swing)
const TRANSITION_TIMING := 0.65  # when this move ends if nothing was queued

func _ready():
	animation = "attack_name"   # <-- set per attack
	move_name = "slash_2"   # <-- must match the key in model.gd's `moves` dict
